...
이 H를 구현할 함수인 A와 B가 있다. A는 덧셈,뺄셈과 같은 어떠한 프로그램이고 B는 A에 입력시킬 입력값이다. ex. A(B) 이 경우에는 아래와 같은 2가지 결과가 나온다.

 1. A와 B를 통해 프로그램이 실행 후 계산이 제대로 끝나게 된다면 TRUE값을 반환한다.

 2. 만약 계산이 끝나지 않게되어 무한루프에 빠지게 된다면 FALSE값을 반환한다.
